...
class PdfService {
  ...
  static Future<void> generateRdpPdf(RdpReport report) async {
    ...
    const pageFormat = PdfPageFormat.a4.landscape;
          ...
          child: pw.Row(
            children: [
                ...
                child: pw.Container(
                  ...
                  child: pw.Row(
                    children: [
                      ...
                      pw.SizedBox(width: 6),
                      pw.Text('LEAR', style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 11, color: _learRed)),
                      pw.SizedBox(width: 4),
                      pw.Text('CORPORATION', style: const pw.TextStyle(fontSize: 8)),
                      pw.SizedBox(width: 16),
                      pw.Text('RDP - Relatório de Produção do Corte', style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 11)),
                    ],
                  ),
                ),
              ),
              pw.Container(
                ...
                decoration: pw.BoxDecoration(border: pw.Border(left: pw.BorderSide(color: _border, width: 0.6))),
  ...
  static pw.Widget _headerField(String label, String value, {int flex = 1}) {
    return pw.Expanded(
      ...
      child: pw.Container(
        ...
        decoration: pw.BoxDecoration(border: pw.Border(right: pw.BorderSide(color: _border, width: 0.4))),
        child: pw.Column(
          crossAxisAlignment: pw.CrossAxisAlignment.start,
          children: [
            pw.Text(label, style: const pw.TextStyle(fontSize: 5.5)),
            pw.Text(value.isEmpty ? ' ' : value, style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold)),
          ],
        ),
      ),
    );
  ...
  static Map<int, pw.TableColumnWidth> _columnWidths(int tm, int tp, int pp, int scrap) {
    // As proporções preservam o desenho do formulário e fazem a tabela ocupar
    // praticamente toda a largura útil da A4 horizontal.
    final base = <double>[
      14, 28, 28, 42, 26, 26, 32,
      ...List<double>.filled(tm, 16),
      ...List<double>.filled(tp, 16),
      ...List<double>.filled(pp, 16),
      ...List<double>.filled(scrap, 18),
      22,
    ];
    final total = base.fold<double>(0, (sum, value) => sum + value);
    final scale = 818 / total;
    return {
      for (var i = 0; i < base.length; i++)
        i: pw.FixedColumnWidth(base[i] * scale),
    };
  }
  ...
  static pw.TableRow _buildGroupHeaderRow() {
    ...
    return pw.TableRow(
      ...
      children: [
        ...
        for (var i = 0; i < tmCount; i++) _cell(i == 0 ? 'TM' : '', bold: true, fontSize: 6, bg: _headerBg),
        for (var i = 0; i < tpCount; i++) _cell(i == 0 ? 'TP' : '', bold: true, fontSize: 6, bg: _headerBg),
        for (var i = 0; i < ppCount; i++) _cell(i == 0 ? 'PP' : '', bold: true, fontSize: 6, bg: _headerBg),
        for (var i = 0; i < scrapCount; i++) _cell(i == 0 ? 'Scrap' : '', bold: true, fontSize: 5.5, bg: _headerBg),
  ...
  static pw.TableRow _buildColumnHeaderRow() {
    ...
    for (final label in TempoMortoCategories.short) cells.add(_vCell(label));
    for (final label in TempoPerdidoCategories.short) cells.add(_vCell(label));
    for (final label in ParadasProgramadasCategories.short) cells.add(_vCell(label));
    for (final label in ScrapRdpColumns.short) cells.add(_vCell(label));
  ...
  static pw.TableRow _buildDataRow(int seq, RdpLine linha, List<String> tm, List<String> tp, List<String> pp, List<String> scrap) {
    final cells = <pw.Widget>[
  ...
  static pw.TableRow _buildEmptyRow(int seq, int tm, int tp, int pp, int scrap) {
    ...
    return pw.TableRow(children: List.generate(totalCols, (i) => _cell(i == 0 ? '$seq' : '', fontSize: 6)));
  }

  static pw.TableRow _buildTotalRow(RdpReport report, List<String> tm, List<String> tp, List<String> pp, List<String> scrap) {
    int totalQtd = 0;
    for (final l in report.linhas) totalQtd += int.tryParse(l.quantidadePecas) ?? 0;
    final cells = <pw.Widget>[
    ...
    for (final key in tm) cells.add(_cell(_totalValue(report.totaisTempoMorto[key]), bold: true, fontSize: 6, bg: _totalBg));
    for (final key in tp) cells.add(_cell(_totalValue(report.totaisTempoPerdido[key]), bold: true, fontSize: 6, bg: _totalBg));
    for (final key in pp) cells.add(_cell(_totalValue(report.totaisParadas[key]), bold: true, fontSize: 6, bg: _totalBg));
    for (final key in scrap) cells.add(_cell(_totalValue(report.totaisScrap[key]), bold: true, fontSize: 6, bg: _totalBg));
    cells.add(_cell('', fontSize: 6, bg: _totalBg));
    return pw.TableRow(children: cells);
  }

  static String _totalValue(int? value) => value != null && value > 0 ? '$value' : '';
  ...
  static pw.Widget _buildRdpFooter(RdpReport report) {
    return pw.Column(
      ...
      children: [
        pw.Container(
          ...
          child: pw.Row(
            ...
            children: [
              ...
              pw.Expanded(child: pw.Text(report.observacoes.isEmpty ? ' ' : report.observacoes, style: const pw.TextStyle(fontSize: 7))),
            ],
          ),
        ),
        ...
        pw.Text('F QUA-E 054 - Relatório de Produção do Corte - ${report.data}', style: const pw.TextStyle(fontSize: 5)),
      ],
    );
  }
  ...
  static pw.Widget _cell(String text, {bool bold = false, double fontSize = 7, PdfColor? bg}) {
    return pw.Container(
      ...
      child: pw.Text(text, style: pw.TextStyle(fontSize: fontSize, fontWeight: bold ? pw.FontWeight.bold : pw.FontWeight.normal), textAlign: pw.TextAlign.center, maxLines: 3),
    );
  }
    ...
    return pw.Container(
      ...
      child: pw.Transform.rotateBox(
        ...
        child: pw.Container(
          width: height - 2,
          alignment: pw.Alignment.center,
          child: pw.Text(text, style: pw.TextStyle(fontSize: 4.2, fontWeight: pw.FontWeight.bold), textAlign: pw.TextAlign.center, maxLines: 4),
        ),
      ),
    );
  }
  ...
  static Future<void> generateScrapPdf(ScrapReport report) async {
    ...
    pdf.addPage(
      pw.Page(
        ...
        build: (context) => pw.Center(child: pw.Text('Registro de Scrap')),
      ),
    );
    await Printing.layoutPdf(
      format: PdfPageFormat.a4.landscape,
      onLayout: (_) async => pdf.save(),
      name: 'SCRAP_${report.data}_${report.maquina}.pdf',
    );
  }
}
